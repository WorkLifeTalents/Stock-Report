...
etf_spx <- read_xlsx(paste(getwd(),"/WLT-Titelliste.xlsx", sep=""), sheet = "ETF-US", .name_repair = "unique_quiet")
```
...
# Calculate sizeref to scale bubble sizes appropriately
# Here, we set a maximum marker size (e.g., 100 pixels) and calculate sizeref accordingly.
max_marker_size <- 100
sizeref <- 2 * max(etf_ch$`Fondsgröße (in Mio. CHF)`) / (max_marker_size^2)

# Define a custom range for the x-axis. Adjust the multiplier as needed.
x_range <- c(min(etf_ch_summary$ann_sd_3y)*0.8, max(etf_ch_summary$ann_sd_3y) * 1.2)

# Create bubble chart
fig <- plot_ly(x = ~etf_ch_summary$ann_sd_3y, 
               y = ~etf_ch_summary$ann_ret_3y, 
               #color = ~etf_ch$Ticker,
               #colors = "Set1",
               type = 'scatter',
               mode = 'markers',
               marker = list(
                 size = ~etf_ch$`Fondsgröße (in Mio. CHF)`,
                 sizemode = 'area',
                 sizeref = sizeref,
                 sizemin = 4  # Minimum bubble size in pixels
               ),
               text = ~paste("Fund:", etf_ch$Fondsname, "<br>AuM:", etf_ch$`Fondsgröße (in Mio. CHF)`)) %>%
  layout(title = "Bubble Chart: Return vs Standard Deviation",
         xaxis = list(title = "3 Jahres ann. Volatilität", range = x_range),
         yaxis = list(title = "3 Jahres ann. Rendite"),
...
# calculate returns & standard deviation
#############################
etf_spx_summary <- summary_ret_sd_table(etf_spx_tbl)
datatable(as.data.frame(etf_spx_summary))
```
